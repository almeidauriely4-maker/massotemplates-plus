...
function Index() {
  return <div className="min-h-screen bg-background text-foreground antialiased">
    <main>
      ...
      <section className="band"><div className="section grid items-center gap-12 lg:grid-cols-2"><img src={easeImageAsset.url} alt="Material MassoPro com +300 sequências em tablet, celular e notebook" className="w-full"/><div><Tag>Sessões planejadas</Tag><h2 className="mt-5 text-4xl font-extrabold sm:text-5xl">Facilidade total</h2><p className="mt-4 text-muted-foreground">Sequências prontas em tempo recorde. Cada sequência inclui tudo que você precisa para uma aplicação técnica perfeita.</p><ul className="mt-7 grid gap-4 sm:grid-cols-2">{features.map(x=><li className="check" key={x}><Check size={17}/>{x}</li>)}</ul></div></div></section>
